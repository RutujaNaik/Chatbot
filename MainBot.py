...
dept_data = []; con_det= []; time_list = [];time_list = []
app_name = []
data_date_list = []
...
######################### Appoinment Booking ###############################
    ...
    response("Success",['success'],single_response= True)
...
#################################
    ...
    if "_name_" in user_input:
       ...
       split_message = re.split(r'\s+|[,;?!.-]\s*', user_input.lower())
        ...
        l3, data_date = BAckend.CSV_Fetching(dept_data[0].strip(), date_det)
        data_date_list.append(data_date)
        ######
        
        if l3 == []:
            data_date_list.clear()
            return f"""Currently Appointment date for {data_date} is not available. Please Book your Appointment for another date for {dept_data[0].strip()} """
        else:
    ...
    elif "_time_" in user_input:
        contact_var = user_input.split("_time_")[0]
        # print("user_input---->",user_input)
        new_user_input = "success"
        user_input = new_user_input
        split_message = re.split(r'\s+|[,;?!.-]\s*', user_input.lower())
        time_list.append(contact_var)
    ...
    temp_data = re.findall(r'(?<=Book your Appointment Date for ).*$', response)
    if temp_data != []:
        dept_data.append((temp_data[0].strip()))
    if data_date_list != [] and data_date_list != [] and con_det != [] and time_list != []:
        print("_"*50)
        'Name', 'Contact details','Division','Day','Time'
        print(app_name[0],con_det[0],dept_data[0].strip(),data_date_list[0],time_list[0])
        BAckend.CSV_Generation(app_name[0],con_det[0],dept_data[0].strip(),data_date_list[0],time_list[0])
        app_name.clear();data_date_list.clear();con_det.clear();dept_data.clear();time_list.clear()
        print("_"*50)
